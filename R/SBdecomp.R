...
sbdecomp = function(outcome, treatment, confounders, data = NULL, type = "inclusion", estimation = "parametric", Bonly = T, balance = T, n.trees = 20000, interaction.depth = 4, shrinkage = 0.005, verbose = FALSE,  stop.method = c("es.max"), cv.folds = 0, standard.error = F, boot.rep=500)
   {#confounders must be in a data.frame with names
	...
	if(sum(is.na(data.check)) > 0) {
		...
		print(paste("Missing values in variable(s):", sss))
		ttt = apply(is.na(data.check),1,sum)
		ttt = which(ttt > 0)
		outcome = outcome[-ttt]
		treatment = treatment[-ttt]
		confounders = confounders[-ttt,]
 		print(paste("Complete cases only used in analysis; ", length(ttt), " cases removed."))
	}
	...
	if(sum(is.f.vec) ==0 ) {print("No confounders identified as factors. If this is incorrect, please check that variables that should be factors are coded as factors in the supplied arguments.")}
	if(sum(is.f.vec) > 0) {print(paste("The following variable(s) were identified as factors:", names.covariates[is.f.vec]))}
	...
	if(length(great.16) > 0) {
		for(tt in great.16) {
			print(paste("Note: Variable name for ",names.covariates[tt]," shortened to ", substr(names.covariates[tt],1,16),sep="" ))
	...
	data.use = as.data.frame(cbind(outcome, treatment, confounders))
	  ...
	  if(type == "removal")	{
	  	print("Note: the nonparamteric estimation procedure is running; this procedure may be very time intensive (on the scale of several minutes to hours depending on the size of your dataset).")
	...
	if(standard.error) {
		...
		print("Note: bootstrap procedure for standard error estimates is running; this procedure may be very time intensive (on the scale of several minutes to a full day depending on the size of your dataset).")
		...
		if(mmm == progress.ind[1]) {print("Bootstrapping is 10% complete.")}
		if(mmm == progress.ind[2]) {print("Bootstrapping is 20% complete.")}
		if(mmm == progress.ind[3]) {print("Bootstrapping is 30% complete.")}
		if(mmm == progress.ind[4]) {print("Bootstrapping is 40% complete.")}
		if(mmm == progress.ind[5]) {print("Bootstrapping is 50% complete.")}
		if(mmm == progress.ind[6]) {print("Bootstrapping is 60% complete.")}
		if(mmm == progress.ind[7]) {print("Bootstrapping is 70% complete.")}
		if(mmm == progress.ind[8]) {print("Bootstrapping is 80% complete.")}
		if(mmm == progress.ind[9]) {print("Bootstrapping is 90% complete.")}
		} #end of boot rep 
